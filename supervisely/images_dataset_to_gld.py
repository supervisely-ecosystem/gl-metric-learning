...
def write_csv_table(csv_path, table_data, is_train=False):
    with open(csv_path, "w") as file:
        ...
        for curr_landmark_id, data in tqdm(
            table_data.items(), total=len(table_data.keys()), desc="writing to file"
        ):
            if len(data) > 0:
...
def show_stats(table_data):
    ...
    for curr_landmark_id, data in tqdm(
        table_data.items(), total=len(table_data.keys()), desc="calculating stats"
    ):
        if len(data) > 0:
            classes_counter += 1
...
for seq_num in [0]:
    ...
    input_data_path = os.path.join(
        dataset_root_path, f"{seq_num}", f"{flag}_image_folder"
    )
    ...
    for index_class_label, class_label in tqdm(
        enumerate(classes_labels), total=len(classes_labels)
    ):  # by every class in image folder ds
        ...
        for image_path in images_paths:  # by every image in current class
            ...
            img_uuid = get_uuid_by_string(
                img_name + class_label + flag
            )  # generate unique image name

            gld_output_class_path = os.path.join(
                output_data_path, f"{img_uuid[0]}/{img_uuid[1]}/{img_uuid[2]}"
            )
            os.makedirs(gld_output_class_path, exist_ok=True)
            gld_output_class_image_path = os.path.join(
                gld_output_class_path, f"{img_uuid}.{img_extension}"
            )
            shutil.copy(
                image_path, gld_output_class_image_path
