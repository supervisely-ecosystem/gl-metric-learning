...
def write_csv_table(csv_path, table_data):
    with open(csv_path, "w") as file:
        ...
        for index, row in tqdm(
            enumerate(table_data), total=len(table_data), desc="writing to file"
        ):
            file.write(f"{row}\n")
...
def show_stats(table_rows):
    ...
    for index, row in tqdm(
        enumerate(table_rows), total=len(table_rows), desc="calc stats"
    ):
...
for index, row in tqdm(
    enumerate(table_rows), total=len(table_rows), desc="filtering_data"
):
