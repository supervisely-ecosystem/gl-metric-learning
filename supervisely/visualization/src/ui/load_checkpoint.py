...
from sly_visualization_progress import (
    get_progress_cb,
    reset_progress,
    init_progress,
    _update_progress_ui,
...
def list_files(sly_fs_path):
    ...
    pth_paths = [
        ...
        if (file["path"].endswith(".pth") or file["path"].endswith(".ckpt"))
        and len(file["path"].split("/")) == nesting_level
    ]
...
def download_checkpoint(sly_fs_checkpoints_path):
    ...
    download_progress = get_progress_cb(
        2, "Download checkpoint", files_size_b, is_size=True, min_report_percent=1
    )
    local_path = os.path.join(g.checkpoints_dir, Path(sly_fs_checkpoints_path).name)
    if not os.path.exists(local_path):
        g.api.file.download(
            g.team_id,
            sly_fs_checkpoints_path,
            local_path,
