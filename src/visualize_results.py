...
def get_embeddings(model, dataloader):
    ...
    with torch.no_grad():
        for input_tensors, target_ids in tqdm(
            dataloader, desc="calculating embeddings"
        ):
...
def process_visualization(outputs_train, outputs_test, current_epoch):
    ...
    functions.save_tensors_unique(
        query_images, tr_ds, pred_index_of_labels, pred_dist, output_path, num_cols=5
    )
...
if __name__ == "__main__":
    ...
    tr_ds = GLRDataset(
        train, normalization=args.normalization, aug=args.test_aug, suffix=".png"
    )
