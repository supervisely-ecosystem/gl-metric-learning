...
def save_tensors_by_indexes(
    query_images, train_dataset, pred_index_of_labels, pred_dist, output_path
):
    for query_index in tqdm(
        range(len(query_images)), total=len(query_images), desc="writing visualizations"
    ):
        curr_tensors_list = []
        ...
        curr_pred_dist = [
            ...
            f"\nscore: {round(float(curr_score), 4)}"
            for index, (curr_score, image_index_in_dataset) in enumerate(
                zip(pred_dist[query_index], curr_pred_index_of_labels)
            )
        ]

        for current_index in curr_pred_index_of_labels:
            curr_tensors_list.append(
                train_dataset.get_original_item(int(current_index))["input"]
                .permute(1, 2, 0)
                .numpy()
            )

        curr_tensors_list.insert(0, query_images[query_index]["image"])
        curr_pred_dist.insert(
            0, f'query image, t: {query_images[query_index]["target"]}'
        )

        save_path = os.path.join(output_path, str(query_index))

        save_image_list(
            list_images=curr_tensors_list,
...
def sort_sum_dist_labels(pred_labels, pred_distances):
    ...
    confs = [
        sum(np.array(pred_distances)[np.array(pred_labels) == x]) for x in unique_labels
    ]
...
def save_tensors_unique(
    ...
    TP = 0
    TP_in_top = 0
    for query_index, q in tqdm(
        enumerate(query_images), total=len(query_images), desc="Calc metrics & plot"
    ):
        query_image, target_label = q["image"], q["target"]
        pred_labels = np.array(
            [
                int(train_dataset.get_original_item(int(x))["target"])
                for x in pred_index_of_labels[query_index]
            ]
        )
        pred_distances = pred_dist[query_index]
        pred_labels_unique, pred_distances_unique = sort_sum_dist_labels(
            pred_labels, pred_distances
        )
        ...
        for l in pred_labels_unique:  # TODO: Map to reference, not train!
            train_img_index = np.array(pred_index_of_labels)[query_index][
                np.argwhere(pred_labels == l)[0]
            ]  # just first match in train
            imgs.append(
                train_dataset.get_original_item(int(train_img_index))["input"]
                .permute(1, 2, 0)
                .numpy()
            )
        ...
        pred_labels_unique = np.insert(
            pred_labels_unique.astype("object"), 0, target_label
        )
        plot_query_result(
            query_index,
            axes,
    ...
    plot_title = (
        f"Acc: {TP/num_rows:.2f}, Top {num_cols - 1} Acc: {TP_in_top/num_rows:.2f}"
    )
    ...
    with open("/root/retail_model_metrics.txt", "a") as file:
        print("functions")
        file.write(
            f"{output_path.split('/')[-1]}, acc: {TP/num_rows:.2f}, top {num_cols - 1} acc: {TP_in_top/num_rows:.2f}\n"
        )


def plot_query_result(
    query_index,
    axes,
    list_images,
    list_confs,
    ...
    for i in range(len(list_axes)):
        ...
        title = (
            f"ID:{list_classes[i]} CONF:{list_confs[i]:.1f}"
            if i > 0
            else f"Target ID:{list_classes[i]}"
        )
        if i == 1:
