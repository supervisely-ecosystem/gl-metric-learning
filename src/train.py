...
class Model(pl.LightningModule):
    def __init__(
        ...
        if args.distributed_backend == "ddp":
            self.num_train_steps = math.ceil(
                len(self.tr_dl)
                / (len(args.gpus.split(",")) * args.gradient_accumulation_steps)
            )
        else:
            self.num_train_steps = math.ceil(
                len(self.tr_dl) / args.gradient_accumulation_steps
            )
        ...
        if args.optimizer == "adamw":
            self.optimizer = AdamW(
                [
                    ...
                    {"params": self.metric_crit.parameters()},
                ],
                lr=self.params.lr,
    ...
    def validation_step(self, batch, batch_nb, dataset_idx):
        if dataset_idx == 0:
            ...
            loss = loss_fn(
                self.metric_crit_val, target_dict, output_dict, val=True
            )  # .data.cpu().numpy()
            ...
            preds_conf_pp, preds_pp = torch.max(
                logits.gather(1, allowed_classes.repeat(logits.size(0), 1)).softmax(1),
                1,
            )
    ...
    def validation_epoch_end(self, outputs):
        ...
        for key in out_tr_filter.keys():
            out_tr_filter[key] = (
                out_tr_filter[key].detach().cpu().numpy().astype(np.float32)
            )
        ...
        pred_dist, pred_index_of_labels = get_topk_cossim(
            out_val["embeddings"], out_tr_filter["embeddings"], k=8, device=device
        )
        ...
        functions.save_tensors_by_indexes(
            query_images, tr_ds, pred_index_of_labels, pred_dist, output_path
        )
        ...
        val_score = comp_metric(
            out_val["targets"], [out_val["preds"], out_val["preds_conf"]]
        )
        val_score_landmarks = comp_metric(
            out_val["targets"],
            [out_val["preds"], out_val["preds_conf"]],
            ignore_non_landmarks=True,
        )

        val_f1, val_precision, val_recall, val_accuracy = functions.calc_metrics(
            predict=out_val["preds"], gt=out_val["targets"]
        )

        val_score_pp = comp_metric(
            out_val["targets"], [out_val["preds_pp"], out_val["preds_conf_pp"]]
        )
        val_score_landmarks_pp = comp_metric(
            out_val["targets"],
            [out_val["preds_pp"], out_val["preds_conf_pp"]],
            ignore_non_landmarks=True,
        )

        val_loss_mean = np.sum(out_val["val_loss"])

        vals, inds = get_topk_cossim(
            out_val["embeddings"], out_tr_filter["embeddings"], k=1, device=device
        )
        ...
        val_score_landmarks_cosine = comp_metric(
            out_val["targets"], [labels, vals], ignore_non_landmarks=True
        )

        f = StandardScaler()
        f.fit(np.concatenate([out_val["embeddings"]], axis=0))
        out_tr_filter["embeddings"] = f.transform(out_tr_filter["embeddings"])
        out_val["embeddings"] = f.transform(
            out_val["embeddings"],
        )
        ...
        val_score_landmarks_sc_cosine = comp_metric(
            out_val["targets"], [labels, vals], ignore_non_landmarks=True
        )
...
if __name__ == "__main__":
    ...
    if args.loss == "bce":
        ...
        metric_crit = ArcFaceLoss(
            args.arcface_s, args.arcface_m, crit=args.crit, weight=class_weights
        )
        metric_crit_val = ArcFaceLoss(
            args.arcface_s, args.arcface_m, crit="bce", weight=None, reduction="sum"
        )

    tr_ds = GLRDataset(
        train, normalization=args.normalization, aug=args.tr_aug, suffix=".jpg"
    )
    ...
    tr_filter_ds = GLRDataset(
        train_filter, normalization=args.normalization, aug=args.val_aug, suffix=".jpg"
    )
    tr_filter_dl = DataLoader(
        dataset=tr_filter_ds,
        batch_size=args.batch_size,
    ...
    if args.logger == "neptune":
        logger = NeptuneLogger(
            ...
            upload_source_files=[
                ...
                "models.py",
                "loss.py",
                f"../configs/{args.experiment_name}.py",
            ],
        )
    ...
    model = Model(
        ...
        metric_crit_val=metric_crit_val,
    ...
    torch.save(
        ...
        experiment_path + "/" + f"{args.experiment_name}_ckpt_{args.max_epochs}.pth",
    )
